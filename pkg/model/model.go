...
type Payment struct {
	...
	Currency     string    `json:"currency" validate:"required,min=3,max=3" db:"currency"`
	...
	PaymentDt    int       `json:"payment_dt" validate:"required,min=10,max=10" db:"payment_dt"`
	...
	CustomFee    int       `json:"custom_fee"  validate:"required" db:"custom_fee"`
}
...
type Order struct {
	OrderUid          string `json:"order_uid" validate:"required,min=15,max=20" db:"order_uid"` //dlina 19
